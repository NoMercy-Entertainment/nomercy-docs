...
`VideoPlayerConfig<T>` extends the kit's [`BasePlayerConfig`](/nomercy-player-core/reference/config) (`baseUrl`, `auth`, `playlist`, `storage`, `platform`, and the rest, shared with the music player). This page covers only the fields video adds. One inherited field is worth flagging here even though video didn't add it. `controls` renders the native `<video controls>` bar. Loading `DesktopUiPlugin` or `TvKeyHandlerPlugin` alongside `controls: true` doubles the UI, so pick one.
...
| `baseImageUrl` | `string` | — | Prefix for relative `image`/`poster`/`thumbnail` paths, useful for a TMDB-style relative path catalog. Absolute URLs pass through unchanged. |
...
Every `setup()` with a non-empty playlist primes a start item once `ready()` resolves. It reads each item's `progress` field, [`WatchProgress`](/nomercy-video-player/reference/types), to pick one.

- Most recently watched wins.
- Over 90% watched rolls forward to the next item.
- Anything else resumes at the saved `time`, passed as `startAt` so the backend fetches from the offset.

This runs regardless of `autoPlay`. Without it, the player still mounts paused on the right item at the right position, rather than blank.

`autoPlay: true` decides the one thing the selection does not: whether that primed item also starts playing.

[Recipe: Persistence & Resume](/nomercy-video-player/recipes/resume-playback) has the full pattern, including saving progress.
