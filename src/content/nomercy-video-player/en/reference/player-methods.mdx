...
`NMVideoPlayer<T>` implements `IVideoPlayer<T>`, which extends the kit's `IPlayer<VideoEventMap<T>>`. Everything on `IPlayer` (`play()`, `pause()`, `time()`, `volume()`, `queue()`, `addPlugin()`, and the rest of transport, queue, volume, and plugin management) is unchanged kit surface. The [Guided Tour](/nomercy-video-player/tour/transport) documents it method by method, so this page does not duplicate it. This page is the delta: everything that exists only because the medium is video.
...
Signatures and behavior are unchanged. See the [Guided Tour](/nomercy-video-player/tour/transport).
