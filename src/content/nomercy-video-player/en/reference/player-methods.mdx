...
| `chapters()` | `() => Chapter[]` | From the item's typed `chapters` field. |
...
`V1VideoCompatPlugin` attaches roughly forty additional method names (`seek()`, `speed()`, `playlist()`, and the rest) onto the instance at runtime, opt-in only, see [Plugin: V1 Compatibility Shim](/nomercy-video-player/plugins-adapters/v1-compat) for the full mapping. None of those are part of the `IVideoPlayer` type surface documented above.

## Next steps

- [Reference: BasePlayerConfig (video)](/nomercy-video-player/reference/config): the `setup(config)` fields specific to video, on top of the kit's shared config.
