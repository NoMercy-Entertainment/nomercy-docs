...
`IVideoPlayer` re-declares a handful of methods that already belong to the shared player surface, so they read next to the video-specific ones.

They add nothing: `time()`, `duration()`, `playbackRate()`, `togglePlayback()`, `toggleMute()`, `rewind()`, `forward()`, `playState()`, `volumeState()`, `volume()`, `buffered()`, `play()`, `pause()`, `stop()`, `next()`, `previous()`, `item()`, `queueLength()`, `index()`, `queue()`, `playbackRates()`.

All are already required on `IPlayer`, with `item()` coming from the shared `WithCurrentItem` capability. The item-bearing ones re-type their playlist values with the concrete video item type `T`.

Signatures and behaviour are unchanged. See the [Guided Tour](/nomercy-video-player/tour/transport).
...
- [Reference: Metrics, Clock & Accessibility](/nomercy-player-core/reference/metrics-and-accessibility): `metrics()`, `now()`, `announce()`, and the `bandwidth()` / `canPlay()` pair [Quality Levels](/nomercy-video-player/tour/quality) already walks through in a video context, shared kit surface not re-declared on `IVideoPlayer`.
