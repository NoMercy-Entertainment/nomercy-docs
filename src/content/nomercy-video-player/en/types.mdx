...
| `imageBasePath`           | `string`                                      | none                                       | Base URL prepended to relative image paths on playlist items |
...
See [Aspect Ratio](/nomercy-video-player/api/aspect-ratio).
...
Player Core base events (`ready`, `time`, `duration`, `play`, `pause`, `ended`, `error`, `subtitle`, `subtitleCue`, `subtitleStyle`, `mediaReady`, etc.) are inherited from `BaseEventMap` and documented at [/nomercy-player-core/event-system](/nomercy-player-core/event-system).
