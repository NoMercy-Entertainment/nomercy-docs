...
Returns the active `IVideoBackend` instance, constructing it on first call. The backend wraps the `<video>` element and the hls.js instance. Most consumers do not need to call this directly — the player's transport and track methods cover the common cases.
...
| `currentTime(t)` | `(t: number) => void` | Seek to position `t` in seconds |
...
| `qualityLevels({ includeUnsupported: true })` | — | All levels including unsupported codecs |
