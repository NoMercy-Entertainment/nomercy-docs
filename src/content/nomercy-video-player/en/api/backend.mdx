...
| `stop`  | `() => void`          | Stop playback and reset position to 0; backend state becomes `paused` (`idle` requires `unload`) |
...
| `currentTime(seconds)` | `(seconds: number) => void` | Seek to position `seconds` in seconds             |
