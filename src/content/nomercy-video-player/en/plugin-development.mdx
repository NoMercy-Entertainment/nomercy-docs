...
When a plugin needs to read the active item with `this.player.item()`, add `& WithCurrentItem<YourItemType>` to the `P` generic.
Plugins that only call transport methods or `metrics()` can omit it — those are on `IPlayer` directly.
...
The type threads through `item()`, `queue()`, and `beforeLoad`:
