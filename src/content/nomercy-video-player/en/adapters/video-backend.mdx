...
Every backend must implement all methods in `IVideoBackend`. See [API — Backend Access](/nomercy-video-player/api/backend) for the full method table.
...
import type { VideoBackendFactory, IVideoBackend, VideoPlayerConfig } from '@nomercy-entertainment/nomercy-video-player';
...
- [Player Kit — writing-a-backend](/nomercy-player-kit/advanced/writing-a-backend)
