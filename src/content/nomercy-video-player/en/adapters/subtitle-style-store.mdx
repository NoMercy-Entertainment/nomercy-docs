...
`ISubtitleStyleStore` is the adapter the player uses to persist and restore the user's subtitle style preferences across page reloads.
The default implementation (`StorageBackedSubtitleStyleStore`) uses the core's `IStorage` adapter so the storage backend is swappable.
...
- [Player Core adapters](/nomercy-player-core/adapters)
