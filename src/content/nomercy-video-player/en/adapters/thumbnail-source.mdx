...
`IThumbnailSource` is the adapter the player calls to supply hover thumbnail frames for the progress bar.
The default implementation (`VttSpriteThumbnailSource`) fetches a WebVTT sprite manifest and parses the sprite coordinates from each cue.
...
`VttSpriteThumbnailSource` is used automatically.
