---
title: v1 → v2 migration (video)
description: Breaking changes and how to upgrade your code from nomercy-video-player v1 to v2.
---
...
For core-level changes (subpath imports, adapter ports, five-layer architecture), see the [core migration guide](/nomercy-player-core/storage).
...
These events no longer exist in v2.
...
Your playlist item type is threaded through the player's type system, so `player.current()` returns `T | undefined` rather than `VideoPlaylistItem | undefined`.
...
v2 follows the same convention: `-1` / `null` means disabled/auto.

`currentQuality()` still returns the HLS.js `currentLevel` index where `-1` means auto-select.
...
v2 introduces 32 named adapter ports (28 from the core + 4 video-specific).
...
const player = nmplayer('player-1').setup({
  ...
  // Video-specific adapters (chapterSource, thumbnailSource, subtitleStyleStore)
  // inject the same way. The defaults read from the playlist item, so you pass
  // one only when you need custom behaviour.
});
```
...
See the [core migration guide](/nomercy-player-core/storage) for the full port catalog and subpath import table.

## Subpath imports

v2 exports each adapter and plugin from a dedicated subpath, enabling tree-shaking.
The root barrel import still works; subpaths are opt-in.
...
The cast receiver is currently v1-based and is slated for a full rewrite.
Migration to v2 should happen as part of that rewrite rather than as an incremental patch.
...
- Core adapter catalog: [Core Adapters](/nomercy-player-core/storage)
