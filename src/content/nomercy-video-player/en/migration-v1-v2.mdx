...
| `playlistItem(idx?)`         | `item()` / `seekToIndex(idx)`                                        |
| `playlistIndex()`            | `index()`                                                            |
| `state()`                    | `playState()`                                                        |
...
`quality()` still returns the HLS.js `currentLevel` index where `-1` means auto-select.
The getter is unchanged.

`subtitle()` returns `number | null` where `null` means off.
