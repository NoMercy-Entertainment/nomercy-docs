...
For kit-level changes (subpath imports, adapter ports, five-layer architecture), see the [kit migration guide](/nomercy-player-kit/storage).
...
v2 introduces 32 named adapter ports (28 from the kit + 4 video-specific). All ports have sensible defaults. You only need to wire an adapter through `setup()` when you want to replace the default behavior.
...
const player = nmplayer('player-1').setup({
  ...
  chapterSource: new VttChapterSource(
...
See the [kit migration guide](/nomercy-player-kit/storage) for the full port catalog and subpath import table.
...
- Kit adapter catalog: [Kit — Adapters](/nomercy-player-kit/storage)
