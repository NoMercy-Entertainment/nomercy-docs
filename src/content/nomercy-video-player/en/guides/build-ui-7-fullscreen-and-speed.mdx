...
> **Note:** `player.fullscreenState()` returns the `FullscreenState` enum (`'on'` / `'off'`). The `fullscreen` event payload carries `{ active: boolean }` so you rarely need to call the getter inside the listener.
...
- `player.playbackRates()` returns the configured rate list (falls back to `[0.5, 0.75, 1, 1.25, 1.5, 2]` when none is set in config).
...
| `player.fullscreenState()` | Returns `FullscreenState.ON` or `FullscreenState.OFF` |
