...
> **Note:** Use `this.listen(target, event, fn)` for DOM listeners so the plugin base removes them automatically on `dispose()`. The native `<video>` `'progress'` listener added directly is one exception covered in `dispose()` below.
...
`'auto'` is the initial state. When the user picks a level, `isAutoQuality` becomes `false` and `selectedQualityIndex` stores which level they chose. The player exposes `player.currentQuality('auto')` to restore adaptive behavior.
...
private createQualityButton(): void {
  ...
  this.on('levels', ({ levels }) => {
    ...
    this.listen(autoOption, 'click', (e) => {
      e.stopPropagation();
      this.player.currentQuality('auto');
    ...
    levels.forEach((level, index) => {
      ...
      this.listen(option, 'click', (e) => {
        e.stopPropagation();
        this.player.currentQuality(index);
...
1. The **Auto button** is always index 0 in the rendered list. It calls `currentQuality('auto')`, which is the v2 API for handing control back to HLS adaptive bitrate.
2. **Manual selection** calls `currentQuality(index)` with a zero-based index into the `levels` array.
...
private rebuildSubtitleMenu(): void {
  ...
  this.listen(offOption, 'click', (e) => {
    e.stopPropagation();
    this.player.currentSubtitle(null);
    this.toggleMenu(null);
  });
  ...
  filteredTracks.forEach((track, index) => {
    ...
    this.listen(option, 'click', (e) => {
      e.stopPropagation();
      this.player.currentSubtitle(index);
      this.toggleMenu(null);
    });
  });
...
> **Note:** `player.currentSubtitle(null)` is the v2 call that turns subtitles off. The older v1 convention of passing `-1` is not the v2 contract. The getter form `player.currentSubtitle()` returns a `CurrentSubtitleSelection` object (with `.index` and `.track`) when a track is active, or `null` when off — not a bare number.

### Subtitle highlighting and icon toggling

`player.currentSubtitle()` returns a `CurrentSubtitleSelection` object (with an `.index` property) when a track is active, or `null` when subtitles are off. The Off button is at DOM index 0; the track buttons start at DOM index 1:

```ts
private highlightCurrentSubtitle(): void {
  if (!this.subtitleMenu) return;

  const current = this.player.currentSubtitle();
...
private createAudioButton(): void {
  ...
  this.on('audioTracks', ({ tracks }) => {
    ...
    tracks.forEach((track, index) => {
      ...
      this.listen(option, 'click', (e) => {
        e.stopPropagation();
        this.player.currentAudioTrack(index);
        this.toggleMenu(null);
      });
    });
...
private highlightCurrentAudio(): void {
  ...
  const current = this.player.currentAudioTrack();
...
use(): void {
  ...
  // so the controls would never receive the paused class. Apply it directly.
...
Document-level listeners need explicit removal because they are not part of the player's element tree. All other cleanup happens automatically through `this.listen()`:

```ts
dispose(): void {
  // this.listen() auto-removes every listener registered through it,
  // including the document click handler above.

  // Remove DOM elements — removing a parent also removes all child listeners.
  this.topBar?.remove();
