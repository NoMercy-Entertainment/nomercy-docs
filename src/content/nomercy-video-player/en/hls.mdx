...
hls.js ships with `nomercy-video-player` as a regular dependency, so installing the player installs hls.js too.
...
The player loads hls.js via dynamic import on first HLS source, so it stays out of your initial bundle until you actually play `.m3u8`.
...
hls.js supports low-latency HLS (LL-HLS).
Configure via `backendFactory` to pass custom hls.js config:
