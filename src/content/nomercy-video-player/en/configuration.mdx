...
The DOM element the player mounts into.
Required for any plugin that renders UI (`DesktopUiPlugin`, `OctopusPlugin`, `SubtitleOverlayPlugin`).
The player itself renders nothing, so the container is only needed by plugins.
...
Async loader called by `player.setLanguage(lang)` when the bundle for that language is not already loaded.
...
Both disable browser native controls.
Use `disableMediaControls` in new code, `disableControls` is a deprecated alias.

## Playlist item fields

### Track kinds

Each playlist item accepts a `tracks` array for sidecar files.
The `kind` field on each track tells the player what to do with the file:
...
The typed alternatives to `tracks[]` are preferred for new integrations.
Use the top-level item fields `subtitles`, `chapters`, `fonts`, and `previewSpriteUrl` directly when the shape is known at build time:
...
The legacy `tracks[]` array is still accepted and normalised automatically.
Tracks with `kind: 'chapters'` are mapped into `item.chapters`; tracks with `kind: 'fonts'` are mapped into `item.fonts`.
