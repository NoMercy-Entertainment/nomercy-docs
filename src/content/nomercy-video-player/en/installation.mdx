...
`hls.js` installs automatically with the player — it is a dependency of `@nomercy-entertainment/nomercy-player-core`, which the player depends on, so `npm install @nomercy-entertainment/nomercy-video-player` pulls it transitively. The player keeps it out of its own bundle (marked `external`, for dedup) and loads it lazily the first time you point at an HLS (`.m3u8`) source. The CDN / IIFE bundle is the one exception: it expects a global `Hls`, so those users load `hls.js` via a separate `<script>` first.
