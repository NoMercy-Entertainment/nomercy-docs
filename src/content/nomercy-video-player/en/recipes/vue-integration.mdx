...
`nomercy-video-player` is not a Vue library. It ships one factory function and a typed method surface, and nothing in it knows what a component is.
...
`VideoPlayer.vue` takes `baseUrl` and `playlist` as props rather than owning them. They are the same two values `useVideoPlayer`'s `config` needs, threaded one level up so whoever mounts the component supplies the real catalog.

`muted: true` and `autoPlay: false` are the constant part. The player loads the first item on mount and waits for a user gesture to play it, like every other runnable example here.

Only `baseUrl` and `playlist` vary by caller:
...
`films` is the real `nomercy-media` fixture catalog (the same three shorts the [Quickstart](/nomercy-video-player/quickstart) and the testbed use), passed down as a prop, not hardcoded inside the component above:
...
When several components need the same instance, a mini-player, a full-screen view, a queue sidebar, provide it once at the root instead of re-deriving state in each.
...
Call `init()` once, after Pinia is installed. Call `dispose()` once, when the app genuinely tears down the player, not on every component unmount. Disposing on every unmount would kill playback each time the user navigates away from the view that happened to call `init()` first:
