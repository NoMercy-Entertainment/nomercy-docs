...
`VideoPlayer.vue` takes `baseUrl` and `playlist` as props rather than owning them. They are the same two values `useVideoPlayer`'s `config` needs, threaded one level up so whoever mounts the component supplies the real catalogue.
