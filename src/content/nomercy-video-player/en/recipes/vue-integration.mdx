...
`baseUrl` and `playlist` below are the real `nomercy-media` fixture catalogue (the same Sintel short film the [Quickstart](/nomercy-video-player/quickstart) mounts) — `muted: true` + `autoPlay: true` so playback starts without a user gesture, exactly like every other runnable example on this site:
