...
`baseUrl` and `playlist` are props. The parent supplies the real `nomercy-media` fixture catalog (the same three shorts the [Quickstart](/nomercy-video-player/quickstart) and the testbed use):
...
`videoPlayerStore` is a module-level singleton, exported once from `stores/videoPlayer.ts`. Every component that imports it, not only `VideoPlayer.svelte` above, reads the exact same reactive state and the exact same player instance. No provider and no second `init()` call is needed:
