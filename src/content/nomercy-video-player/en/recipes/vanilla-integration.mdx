...
{/* Source: packages/player-web/nomercy-video-player/src/index.ts */}
...
[Vue](/nomercy-video-player/recipes/vue-integration), [React](/nomercy-video-player/recipes/react-integration) and [Svelte](/nomercy-video-player/recipes/svelte-integration) each wrap this same pattern behind their own reactivity primitive.
...
`films` is the real `nomercy-media` fixture catalog (the same three shorts the [Quickstart](/nomercy-video-player/quickstart) and the testbed use). The markup below already has a title, a progress slider, and a play/pause button in it. These are the same three elements every framework recipe in this section renders. `createPlayerController`'s snapshot just drives them directly, instead of through a template:
...
A page might mount several independent controllers against the same player: a status readout, a queue panel, a volume indicator. It can tear any one of them down without touching the others, or the player instance they all share.
...
:::snippet{file="video-recipe-vanilla-integration" live="false"}
:::
