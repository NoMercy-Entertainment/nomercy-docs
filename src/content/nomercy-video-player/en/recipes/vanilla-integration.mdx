...
`films` is the real `nomercy-media` fixture catalogue (the same three shorts the [Quickstart](/nomercy-video-player/quickstart) and the testbed use). The markup below already has a title, a progress slider, and a play/pause button in it. These are the same three elements every framework recipe in this section renders. `createPlayerController`'s snapshot just drives them directly, instead of through a template:
