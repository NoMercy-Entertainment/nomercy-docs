...
`'item'` fires when the cursor moves, `'queue'` fires when the array itself changes (append, remove, reorder, replace). A panel that re-renders on both never drifts from the real state, and never needs a poll loop:
