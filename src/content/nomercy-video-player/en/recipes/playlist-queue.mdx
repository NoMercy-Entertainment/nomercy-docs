...
`'current'` fires when the cursor moves, `'queue'` fires when the array itself changes (append, remove, reorder, replace). A panel that re-renders on both never drifts from the real state, and never needs a poll loop:
...
Every mutation method fires its own typed event, so a panel that already re-renders on `'queue'` gets append/remove/reorder for free, no extra listener needed:

```ts
addToQueueButton.addEventListener('click', () => {
  player.queueAppend(newItem); // fires 'queue' — panel re-renders itself
});
