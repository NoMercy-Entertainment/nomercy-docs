...
Chapters are provided per item; the player core's media-tracks layer reads the inline `chapters` array (or a sidecar `kind: 'chapters'` VTT).
...
The player reads chapters from the `chapters` array on each item.
