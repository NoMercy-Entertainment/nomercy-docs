...
`playlist` is a prop, not hardcoded — `VideoPlayerView` above takes real playlist data from whoever renders it. `films` is the real `nomercy-media` fixture catalogue (the same three shorts the [Quickstart](/nomercy-video-player/quickstart) and the testbed use):
