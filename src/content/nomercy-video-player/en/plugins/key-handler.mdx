...
It subclasses the kit's base `KeyHandlerPlugin` and adds the full v1 binding set on top of the kit's cooldown, scope, `when`, and cleanup infrastructure.
...
`KeyHandlerPlugin` inherits `KeyHandlerOptions` from the kit base class.
...
These public methods are inherited from the kit base class.
