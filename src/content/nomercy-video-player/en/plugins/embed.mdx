...
| `applyIframeTweaks` | `boolean` | `undefined` | Declared in the options interface but not yet read by the implementation. Has no effect at runtime. |
...
The player's real event payload is the `data` value.
Extra fields described below are properties of `data`, not top-level fields on the message.

| Forwarded name | `data` shape | Description |
| -------------- | ------------ | ----------- |
| `'ready'` | `void` | Player has initialised |
| `'play'` | `ActionOptions` (`{ source?, silent?, autoplay? }`) | Playback started |
| `'pause'` | `ActionOptions` (`{ source?, silent?, autoplay? }`) | Playback paused |
| `'ended'` | `void` | Track finished |
...
| `'error'` | `PlayerErrorEvent` | Player error (not in the default `forwardEvents` list; add explicitly). `data.error.code` holds the error code; `data.error.message` holds the message. |
...
window.addEventListener('message', (event) => {
  ...
  switch (msg.name) {
    ...
    case 'error':
      // msg.data = PlayerErrorEvent
      console.error('player error', msg.data.error.code, msg.data.error.message);
      break;

    case 'play':
    case 'pause':
      // msg.data = ActionOptions = { source?, silent?, autoplay? }
      console.log(msg.name, 'triggered by', msg.data.source);
      break;
  }
});
```
...
- [Player Kit, built-in plugins reference](/nomercy-player-kit/plugins-builtin)
