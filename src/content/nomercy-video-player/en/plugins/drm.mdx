...
When `use()` runs, the plugin calls `requestMediaKeySystemAccess` to confirm EME is available.
If the API is missing, it emits `'unsupported'` and returns without registering any listeners.

Once EME is confirmed, the plugin subscribes to the player's `'current'` event.
Whenever the active item has a `drm` field, it calls `requestMediaKeySystemAccess` again with the item's key system to verify reachability.
...
Internally it uses the kit's `this.fetch` helper, so it inherits the player's auth and retry configuration.
...
The underlying HTTP call uses the kit's `this.fetch` helper, not the platform `fetch` directly.
