...
[Step 2](/nomercy-video-player/build/play-pause) wired playback. This step adds the control everyone reaches for first: a scrubber. It sits inside the bottom bar, above the button row; `flex-col` on the bar from step 1 was for exactly this. The scrubber is four elements: the bar, a buffer fill, a progress fill, and a hover nipple.
...
While `isMouseDown` is true, the `time` listener that keeps the bar in sync during normal playback skips its updates. Otherwise every tick would yank the nipple back to the playhead mid-drag:
...
The `time` event carries the player's full time snapshot: `time` and `position`, `duration`, `buffered`, `remaining`, and a ready-made `percentage`. It is the same shape `timeData()` returns, so the handler does no time math.

The progress fill and nipple take `percentage` as it comes.

The buffer fill is the one derived value, because "how far ahead is buffered" is a UI choice. `buffered` is seconds ahead of `position`, so the fill ends at `position + buffered`.
