...
{/* Source: docs/nomercy-examples/src/components/TutorialPlayer/steps/step3Plugin.ts, packages/nomercy-player-core/src/types/player.ts */}
...
The `time` event carries seconds; the percentage is yours to derive from `duration()`. And when the playlist moves to another item, both fills reset to zero through the same `item` event the title bar will use in [step 6](/nomercy-video-player/build/title-bar):
