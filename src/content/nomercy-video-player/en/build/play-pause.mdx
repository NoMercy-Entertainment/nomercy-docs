...
Controls need icons, and icons are data, not code. The plugin keeps them in one module-level table. Each entry holds a normal and a hover variant of the same glyph, using the design system's own paths, the ones the shipped plugin renders. One renderer stacks both variants into an inline SVG:
...
The path data is elided here. The full example at the bottom carries every entry verbatim, and each later step adds the entries its controls need. Nothing reads `title` yet; it just documents what the glyph depicts. The outline-to-filled swap on hover is three utility classes on the button, coming right up.
...
`togglePlayback()` flips between playing and paused from the player's own state. Your click handler needs no `play()` and `pause()` branching.

The button does not track its own visibility with a flag either. It listens.

Anything that changes the state fires the same `play` event, and the button hides itself. A click does it, so does a keyboard shortcut, and so does a remote-control command over NoMercy Connect:
...
Step 1's bottom bar stays exactly as built: a column. This step nests a horizontal row inside it, the home for every control button. Step 3 will drop the progress bar into the same column, above this row. That is the whole reason the bar is `flex-col`:
