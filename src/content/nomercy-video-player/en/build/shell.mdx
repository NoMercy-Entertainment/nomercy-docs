...
This is a ten-step tutorial where you write your own control UI, one piece at a time. It is the v2 translation of the original tutorial on examples.nomercy.tv, step for step: a shell, play and pause, a progress bar, time and skip, volume, a title bar, fullscreen and speed, then the shipped plugin and what it adds on top. When you finish, you will understand every line the real `DesktopUiPlugin` runs, because you will have written its shape yourself.
...
- `playing` / `paused` / `stopped` / `ended` / `buffering`, the playback state, mutually exclusive
