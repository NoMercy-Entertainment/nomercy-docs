...
DOM-construction helpers (`createElement`, `createButton`, `createSVG`, `addClasses`, `removeClasses`) live on the `Plugin` base class, not the player, so every call here is bare `this.*`: `this.mount()` claims the wrapper, `this.addClasses()` styles it. `this.player.container` is the one place `this.player` shows up, reaching the collaborator this plugin extends to read its DOM node. That split, what you *are* versus what you *talk to*, is the organising rule of the whole trio and decides where every behaviour lives. [Player and Plugin](/nomercy-player-core/tour/composition-boundary) is the short read.
