...
`mousemove` over the step-3 slider bar does four things. It converts the pointer position to a time with the same math the scrubber uses, finds the covering cue, paints the tooltip by shifting the sprite's `background-position`, and looks up the covering chapter for the title line.
