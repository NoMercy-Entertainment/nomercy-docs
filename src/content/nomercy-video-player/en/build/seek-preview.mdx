...
{/* Source: docs/nomercy-docs/src/examples/video-build-9-seek-preview.ts, packages/player-web/nomercy-player-core/src/core/plugin/base.ts, packages/player-web/nomercy-player-core/src/types/chapter.ts */}
...
Both are fields on the playlist item from [step 1](/nomercy-video-player/build/shell):

| Field | What it holds |
| --- | --- |
| `previewSpriteUrl` | Names a VTT manifest whose cues map time ranges to regions of one sprite image (`thumbs.webp#xywh=x,y,w,h`). |
| `chapters` | Carries `{ index, start, end, title }` entries. The player exposes them through `chapters()` and announces changes through the `chapters` event. |

This step is where both fields light up.
...
`this.fetch()` is the plugin base's auth-aware fetch. It resolves the URL through the player's auth config, attaches credentials when configured, retries per policy, and ties the request's lifetime to the plugin.

The sprite manifest is plain text, so the default text response is right.

`scope: 'silent'` keeps a missing manifest from surfacing as a player error. No thumbnails is a fine state:
...
The parser is module-level code. It works in steps:

1. Split cue blocks.
2. Read the `-->` timestamps.
3. Match the `#xywh=` region.
4. Resolve the sprite image against the VTT's own URL.

Sprite paths are relative to their manifest, like every sprite format:
...
`mousemove` over the step-3 slider bar does four things on every move:

1. Converts the pointer position to a time, with the same math the scrubber uses.
2. Finds the covering cue.
3. Paints the tooltip by shifting the sprite's `background-position`.
4. Looks up the covering chapter for the title line.

Clamping measures the card's real `offsetWidth`, so the tooltip stays inside the bar at both ends. Leaving the bar fades it out:
...
:::snippet{file="video-build-9-seek-preview"}
:::
