...
To swap the entire queue while the player is running, call `player.queue(newItems)` to replace the list, then `player.current(0, { autoplay: true })` to jump to the first item and start playback.
`queue()` accepts `VideoPlaylistItem[]`; `current()` accepts an item, an id string, or a numeric index.
...
function loadNewPlaylist(items: VideoPlaylistItem[]) {
  ...
  player.value?.current(0, { autoplay: true });
}
```
...
> **Note:** Calling `queue()` replaces the list immediately but does not start loading. The `current(0, { autoplay: true })` call triggers the load and playback. If you want to append items to a running queue instead, use `player.queueAppend(items)`.
