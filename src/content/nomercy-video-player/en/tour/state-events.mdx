...
Everything below is unique to `VideoEventMap`, layered on top of the kit's `BaseEventMap` (transport, queue, volume, i18n — the events every tour page so far has already covered).
...
Every event on this page fires against the player below as it plays.

:::snippet{file="video-tour-state-events" live="false"}
:::
