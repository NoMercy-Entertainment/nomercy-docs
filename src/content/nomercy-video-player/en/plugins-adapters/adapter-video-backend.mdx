...
Native HLS on Safari, `hls.js` everywhere else. The import is dynamic, so the bundle cost is zero on Safari, and `player.play()` never branches on browser.

It applies HDR-aware level constraints itself, capping or lifting HLS renditions as the display's HDR capability changes.

It emits the full `BackendEventPayload` map, which `NMVideoPlayer` bridges onto its own public events: `levels`, `level-switched`, `audioTracks`, `stream:error`, `stream:recovering`, `subtitleCue`, and the standard media element events.
...
- A swap is not a crossfade. Whatever was playing stops, and the new backend comes up with no source, so reload to resume. See [Reference: Video Player Methods](/nomercy-video-player/reference/player-methods#video-element--geometry). Cast handoff is a separate concern, through `transferTo()` at plugin level.
