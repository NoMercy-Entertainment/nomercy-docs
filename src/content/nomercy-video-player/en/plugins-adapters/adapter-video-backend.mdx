...
- Only one backend is active per player at a time. Swapping backends mid-session (Cast handoff, for instance) is a `transferTo()` / plugin-level concern, not something `IVideoBackend` itself supports.
