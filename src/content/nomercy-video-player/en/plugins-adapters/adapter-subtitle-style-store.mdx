...
`StorageBackedSubtitleStyleStore` persists through the kit's [`IStorage`](/nomercy-player-core/plugins-adapters/adapter-storage) port, the same swappable storage interface volume and quality preferences already use, under the key `'subtitle-style'` (namespaced automatically when the storage backend itself is namespaced):
