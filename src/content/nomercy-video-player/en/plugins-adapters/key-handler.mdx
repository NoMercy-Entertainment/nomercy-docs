...
- `shift+?` is bound as `'shift+?'`, not `'?'`. Pressing `?` on a standard keyboard sends `key='?'` with `shiftKey=true`, and the kit's canonicalizer folds modifier state into the key string, so the literal `'?'` would never match.
