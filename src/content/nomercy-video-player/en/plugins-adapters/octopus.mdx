...
Fonts come from the item's typed `fonts` field, either a `fonts.json` manifest URL or a list of direct font file URLs, see [`FontTrackRef`](/nomercy-video-player/reference/types). Manifest entries and direct entries can both be present; the plugin fetches whichever is there, resolves each through the player's `resolveUrl` pipeline (so `baseUrl` and auth apply), and falls back to the plugin-level static `fonts` option when the item has none.
