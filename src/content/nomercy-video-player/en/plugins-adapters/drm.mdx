...
The plugin listens for the player's `'item'` event. When the incoming item carries a `drm.keySystem` hint (a consumer-supplied field on your own item shape, not part of the canonical `VideoPlaylistItem`), the plugin attempts the EME handshake for that key system. When the item doesn't specify one, the plugin falls back to its own `keySystem` option:
...
- All license network I/O goes through the kit's `Plugin.fetch()`, the same auth pipeline every other plugin uses. The `auth` config from `setup()` (bearer token, custom headers, request signing) already applies without extra config here.
