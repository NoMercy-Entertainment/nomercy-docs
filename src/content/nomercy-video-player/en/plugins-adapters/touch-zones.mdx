...
- Mobile detection is a one-shot capability check at startup, `ontouchstart` or `maxTouchPoints > 0`, not a live pointer-type check. A touch-capable laptop driven with a mouse still gets the mobile volume zones.
