...
Get the active subtitle track index (`null` = off), or set by index.
...
Equivalent to `currentTime(chapters()[idx].start)`.
...
Shorthand for `fullscreenState(!isActive)`.
...
`CanPlayResult` extends `MediaCapabilitiesDecodingInfo`.
...
Hand off playback to a cast target.
