...
A first-class group-listening plugin is roadmapped for v2.1 to replace the manual coordination above. It is not available yet — today it exists only as a reserved stub that throws `NotImplementedError` — so build on the manual patterns in this guide for now. They stay valid once the plugin lands.
