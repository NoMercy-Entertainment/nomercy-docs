...
Read this when bundle size, memory usage, or long-session stability matter to your app.
Covers tree-shaking, subpath imports, memory budgeting, and verifying there are no leaks.
...
hls.js is the largest dependency in the video player.
Configure your bundler to split it into a separate chunk so the player core loads immediately and hls.js loads async:
...
- [Core: Testing](/nomercy-player-core/testing), `assertNoListenerLeak` and the full test harness
- [Advanced: Multi-Instance](/nomercy-player-core/advanced/multi-instance), memory budget multiplies per instance
