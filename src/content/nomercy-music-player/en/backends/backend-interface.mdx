...
Backend events are forwarded to the player's event bus by `_wireBackend`. You typically subscribe to player-level events (`player.on(...)`) rather than backend-level events directly.

`BackendEvent` values: `'loadstart' | 'loadedmetadata' | 'canplay' | 'play' | 'playing' | 'pause' | 'ended' | 'timeupdate' | 'waiting' | 'stalled' | 'ratechange' | 'encrypted' | 'error'`
