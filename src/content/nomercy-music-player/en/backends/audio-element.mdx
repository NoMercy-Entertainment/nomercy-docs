...
Uses a native `<audio>` element for transport and optionally creates a `MediaElementAudioSourceNode` when a plugin requests the Web Audio graph.

**Kind:** `'audio-element'`

## Activation

This backend is active by default.
...
The backend creates `MediaElementAudioSourceNode` only when the first plugin calls `outputNode(ctx)` or `analyserSource(ctx)`.
Until then, the `<audio>` element routes directly to the system output.
...
Crossfade is driven by a `requestAnimationFrame` loop at ~50 fps.
A secondary `<audio>` element is allocated via `loadSecondary()`, pre-rolled via `primeSecondary()`, and then both elements are gain-ramped in opposite directions over `durationMs`.
...
`setSinkId` is available in Chrome and Chromium-based browsers.
