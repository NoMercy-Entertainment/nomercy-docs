...
`MusicPlayer.vue` below takes `baseUrl` and `playlist` as props instead of owning them. These are the same two values `useMusicPlayer`'s `config` already needs, just threaded one level up so whoever mounts the component supplies the real catalog:
...
`player` is a top-level `<script setup>` ref, so the template above reads it unwrapped: `player?.togglePlayback()`, no `.value`. Vue does that unwrapping automatically for refs returned from a composable and used directly in the template.

Inside `<script>` itself, in the `seek()` handler, `player` stays an explicit ref: `player.value?.seekByPercentage(...)`.

## Mounting it

`songs` is the real `nomercy-media` fixture catalog, the same Ketsa/bent wyre tracks the [Quickstart](/nomercy-music-player/quickstart) and the testbed use. It is passed down as a prop, not hardcoded inside the component above:
...
Several components can need the same player instance, for example:

- A mini-player
- A queue sidebar
- A lyrics panel

When that happens, provide the instance once at the root instead of re-deriving state in each component.

Unlike the composable above, this is safe at `main.ts`'s top level rather than inside `onMounted`.

`#global-player` is a static element in `index.html`, a sibling of `#app` that Vue never renders. It exists in the DOM before this module runs:
...
Each store method has its own moment to run:

| Method | Call it |
| --- | --- |
| `init()` | Once, after Pinia is installed. |
| `dispose()` | Once, when the app genuinely tears down the player. |

Do not call `dispose()` on every component unmount. That would kill playback every time the user navigates away from the view that happened to call `init()` first:
