...
`MusicPlayer.vue` below takes `baseUrl` and `playlist` as props instead of owning them. These are the same two values `useMusicPlayer`'s `config` already needs, just threaded one level up so whoever mounts the component supplies the real catalogue:
...
`songs` is the real `nomercy-media` fixture catalogue, the same Ketsa/bent wyre tracks the [Quickstart](/nomercy-music-player/quickstart) and the testbed use. It is passed down as a prop, not hardcoded inside the component above:
