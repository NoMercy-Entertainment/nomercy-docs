...
Read this when you need smooth music transitions, either a volume-faded crossfade or a preloaded gapless handoff.
This applies to the music player only.
...
- [Crossfade, full `crossfadeTo()` API and per-call options](/nomercy-music-player/crossfade)
- [AutoAdvancePlugin, `crossfade`, `preloadNext`, and `crossfadeDuration` options](/nomercy-music-player/plugins/auto-advance)
- [Backends overview, `audio-element` vs `webaudio` crossfade trade-offs](/nomercy-music-player/backends/overview)
- [Lyrics and Equalizer, EQ applies through crossfade transitions](/nomercy-music-player/recipes/lyrics-and-equalizer)
