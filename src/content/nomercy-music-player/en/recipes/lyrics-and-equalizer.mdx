...
Read this when you are building a Now Playing screen that shows synced lyrics and includes an equalizer.
Both are opt-in, so include only what you need.
...
The EQ graph applies to both the primary and secondary audio paths during a crossfade, so no extra handling is required.
