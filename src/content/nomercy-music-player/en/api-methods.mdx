...
### `currentTime(seconds, opts?)`

```ts
currentTime(seconds: number, opts?: ActionOptions): Promise<void>
```
...
Skip forward by `seconds` seconds (default: 10).
...
Skip backward by `seconds` seconds (default: 10).
...
Dispose and remove a plugin.
Throws if other plugins depend on it (unless `cascade: true`).
