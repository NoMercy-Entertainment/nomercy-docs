...
```ts
// payload: { kind: AudioBackendKind }
```

Fires when `backend(kind)` completes a backend swap. `kind` is `'audio-element'` or `'webaudio'`.

This event is emitted by the player but is not present in `MusicEventMap`. Listen to it with a cast when you need it:

```ts
player.on('backend:changed' as any, ({ kind }: { kind: 'audio-element' | 'webaudio' }) => {
