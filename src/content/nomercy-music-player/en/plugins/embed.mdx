...
| `applyIframeTweaks` | `boolean` | `true` when inside a nested browsing context | Declared in `EmbedOptions` but not read by `use()` at runtime. Has no effect in the current release. |
...
| `'error'` | `PlayerErrorEvent` (`error.code` at `data.error.code`, `error.message` at `data.error.message`) | Player error (not forwarded by default) |
...
window.addEventListener('message', (event) => {
  ...
  if (name === 'error') {
    console.error('player error:', data.error.code, data.error.message);
  }
});
```
