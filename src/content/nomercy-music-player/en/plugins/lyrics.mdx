...
On every `current` event it resolves the lyric URL, fetches the file through the kit's auth-aware fetch, runs the content through the cue parser registry, and hands the result to a `CueTracker` that fires `line`, `lineEnter`, and `lineExit` events in sync with playback.
...
`.lrc` and `.vtt` parsers are pre-registered by the kit.
...
| `plugin:lyrics:enabled` | `{ id: string }` | Fired by the base plugin when `plugin.enable()` is called. |
| `plugin:lyrics:disabled` | `{ id: string; reason?: string }` | Fired by the base plugin when `plugin.disable(reason?)` is called. |
