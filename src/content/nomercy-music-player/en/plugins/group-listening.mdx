...
description: 'Synchronized multi-client listening, roadmapped for v2.1. The public API surface is reserved and exported. Calling use() throws NotImplementedError internally; the kit catches the error and emits plugin:failed.'
draft: false
---
...
The kit catches that error inside `_registerPlugin`, logs it, calls `dispose()`, and emits `plugin:failed` and `plugin:group-listening:failed`.
...
The kit catches this internally; the exception does not reach your code.
...
Register the plugin and listen to `plugin:failed` to react when the kit disables it.
Do not wrap `addPlugin` or `setup` in a try/catch expecting `NotImplementedError` -- the kit swallows that error internally and it never reaches your code.
...
// The kit emits plugin:failed when use() throws internally.
