...
Tag-aware shuffle within the library. Avoids repeating the same artist or album within a configurable window. When tag overlap is not enough to pick a good next track, it delegates to a [Similarity Engine](/nomercy-music-player/adapters/similarity-engine) if one is wired.
