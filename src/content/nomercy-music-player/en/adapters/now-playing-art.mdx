...
```ts
import type { INowPlayingArt } from '@nomercy-entertainment/nomercy-music-player';
```
...
```ts
import { MediaSessionArtProvider } from '@nomercy-entertainment/nomercy-music-player';
```
...
- [MusicPlaylistItem](/nomercy-music-player/playlist-item) — `cover`, `name`, `artist_track` fields
