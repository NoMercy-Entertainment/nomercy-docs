...
Port for resolving lyrics URLs, the `ILyricSource` contract. `LyricsPlugin` calls `resolve(item)` on each track change and passes the returned URL to the player core's cue parser registry for fetching and parsing.
