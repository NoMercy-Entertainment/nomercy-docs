...
The current position is lost, so start playback explicitly after switching.
...
- [Backends overview](/nomercy-music-player/backends/overview), for when to pick each backend
- [IAudioBackend](/nomercy-music-player/backends/backend-interface), the full interface reference
- [Configuration](/nomercy-music-player/configuration), the `backend` and `backendFactory` options
