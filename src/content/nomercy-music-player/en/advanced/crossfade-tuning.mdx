...
The `audio-element` backend uses a `requestAnimationFrame` loop (~50 fps). One frame is ~20 ms. Gain steps are applied every frame, so transitions are not sample-accurate — slight audible stepping may occur on short fades.
...
```ts
await player.crossfadeTo(nextTrack, { duration: 12, curve: 'equal-power' });
```
...
- [Backends overview](/nomercy-music-player/backends/overview) — backend accuracy comparison
