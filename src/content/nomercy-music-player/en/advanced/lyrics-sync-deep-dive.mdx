...
4. The core walks the registered `CueParser[]` in registration order. The first parser whose `canParse(url)` returns `true` wins.
...
The cue list is an array of `Cue<T>` objects with `startMs` and `endMs` fields.
