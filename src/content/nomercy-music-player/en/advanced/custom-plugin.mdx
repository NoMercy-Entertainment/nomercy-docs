...
For the general plugin authoring guide (lifecycle, `this.on`, `this.emit`, `this.fetch`, `this.logger`), see [Writing Plugins](/nomercy-player-core/plugin-authoring) in the player core docs.
...
- [Player Core, Writing Plugins](/nomercy-player-core/plugin-authoring) for the full plugin authoring guide (lifecycle, helpers, i18n)
- [IAudioBackend](/nomercy-music-player/backends/backend-interface) for methods available via `this.player.backend()`
- [Events](/nomercy-music-player/events) for `MusicEventMap` events available via `this.on(...)`
