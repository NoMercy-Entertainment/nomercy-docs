  ...
  override use(): void {
    ...
    this.on('current', ({ item }) => {
      if (item) {
        ...
        console.log('Artist:', item.artist_track?.[0]?.name);
      }
    });
...
override use(): void {
  this.on('ended', () => {
    ...
    void this.player.crossfadeTo(next, { duration: 3, curve: 'equal-power' })
...
export class BeatDetectorPlugin extends Plugin<NMMusicPlayer<any>, {}, BeatEvents> {
  ...
  private async detectBeat(): Promise<void> {
    // … analysis …
    this.emit('beat:detected', { bpm: 128, confidence: 0.92 });
  }
}

// Usage:
player.on(BeatDetectorPlugin, 'beat:detected', ({ bpm, confidence }) => {
