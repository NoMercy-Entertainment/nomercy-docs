...
EQ state (current bands, preset name, preGain) is automatically persisted to the configured storage backend.
On the next `player.setup()`, `EqualizerPlugin` restores from storage.

The default storage backend is `LocalStorageBackend`.
Override in setup for different persistence characteristics:
