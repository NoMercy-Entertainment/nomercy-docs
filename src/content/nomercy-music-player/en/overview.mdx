...
description: 'The NoMercy Music Player: headless, plugin-driven, event-driven. Architecture overview and relation to the player core.'
draft: false
---

# What is the NoMercy Music Player

`@nomercy-entertainment/nomercy-music-player` is a headless, TypeScript-first audio engine for browsers.
It does the audio work, playback, queue, sample-accurate crossfade, lyric sync, and EQ, and ships no UI of its own.
That is deliberate: NoMercy plays the same library on a web dashboard, a phone, and a cast receiver, and a single opinionated UI component would fight all three.
So every visual element is a plugin or something you build, and you wire it to plain events and methods.
You get the full engine; the look and feel is yours.

## Architecture

The player sits on top of the **NoMercy Player Core** (`@nomercy-entertainment/nomercy-player-core`).
The core owns:
...
- [Player Core](/nomercy-player-core/): shared infrastructure documentation
