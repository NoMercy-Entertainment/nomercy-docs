...
v2 introduces 34 named adapter ports (28 from the kit + 6 music-specific). All ports have sensible defaults. You only need to wire an adapter through `setup()` when you want to replace default behavior.
...
See the [kit migration guide](/nomercy-player-kit/adapters) for the full 28-port kit catalog and subpath import table.
