...
## `'backend:changed'` event not typed

**Symptom:** TypeScript error when listening to `'backend:changed'`.

This event is emitted but not declared in `MusicEventMap`. Use a cast:

```ts
player.on('backend:changed' as any, (e: { kind: string }) => {
  console.log('Backend switched to:', e.kind);
});
```

This is a known gap. The event is real and fires on every successful `backend(kind)` swap.
