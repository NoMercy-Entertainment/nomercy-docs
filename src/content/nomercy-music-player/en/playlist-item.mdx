---
title: 'MusicPlaylistItem'
description: 'All fields of MusicPlaylistItem — id, name, url, cover, artist_track, album_track, lyricsUrl, duration — with typing notes and real examples.'
category: 'Playlist Item'
order: 28
draft: false
---

# MusicPlaylistItem

The default track shape for `NMMusicPlayer`. Every item in the `playlist` array must satisfy this interface (or a type that extends it).

```ts
import type { MusicPlaylistItem } from '@nomercy-entertainment/nomercy-music-player';
```

---

## Interface

```ts
interface MusicPlaylistItem extends BasePlaylistItem {
  // Required
  id: string | number; // from BasePlaylistItem
  name: string;

  // Optional
  url?: string;
  cover?: string;
  artist_track?: ArtistRef[];
  album_track?: AlbumRef[];
  lyricsUrl?: string;
  duration?: number;
}

interface ArtistRef {
  id: string | number;
  name: string;
}

interface AlbumRef {
  id: string | number;
  name: string;
}
```

---

## Fields

### `id` (required)

```ts
id: string | number;
```

Unique identifier for the track. Used by the player to locate items in the queue (`queueRemove`, `queueIndexOf`, `current`). Must be unique within a session.

### `name` (required)

```ts
name: string;
```

Track title. Displayed by `MediaSessionPlugin` on lock screens and system media controls. `name` is required on music items — unlike the video player's `title?`, this field has no optional fallback.

### `url`

```ts
url?: string
```

Audio source URL. Supports:

- Direct MP3/AAC/FLAC/OGG: `https://cdn.example.com/track.mp3`
- HLS playlist: `https://cdn.example.com/stream.m3u8` — `hls.js` is loaded automatically when native HLS is unsupported
- Relative URL: resolved against `baseUrl` from config

When `url` is absent, the item can still be in the queue (the player skips it on natural advance and throws on an explicit `load()` call).

### `cover`

```ts
cover?: string
```

Album art URL. Consumed by `MediaSessionPlugin` (pushes to OS lock screen), `CastSenderPlugin` (included in `MusicTrackMediaMetadata`), and `MusicUiPlugin` (renders in the UI overlay).

### `artist_track`

```ts
artist_track?: ArtistRef[]
```

Contributing artists. Each entry has `id` and `name`. Multiple entries for compilations or features.

`MediaSessionPlugin` joins all names with `, ` to produce the `artist` field for OS media controls.

### `album_track`

```ts
album_track?: AlbumRef[]
```

Album associations. Each entry has `id` and `name`. The first entry (`album_track[0].name`) is used as the album name in `MediaSessionPlugin` metadata.

### `lyricsUrl`

```ts
lyricsUrl?: string
```

URL of an LRC lyrics file for this track. `LyricsPlugin` reads this field when `autoFetch: true` (the default). On each `current` event, the plugin fetches the URL and parses it through the kit's cue parser registry.

If you need a different resolution strategy (e.g. looking up lyrics from a server API by track ID), pass a `getLyricsUrl` function in `LyricsPlugin` options instead of populating this field per item.

### `duration`

```ts
duration?: number
```

Track duration **in seconds**. This is a metadata-only fallback used by the player before the audio backend resolves the real duration from the file. When `loadedmetadata` fires and the backend returns `duration > 0`, the backend value takes precedence and this field is ignored.

Use it to populate seekbars immediately on track load without waiting for the audio metadata probe to complete, especially on slow connections.

```ts
{
  id: 'kjc-01',
  name: 'Thaw You Out',
  url: 'https://cdn.example.com/01.mp3',
  duration: 204, // 3:24 — seekbar renders immediately
}
```

---

## Custom fields

The player is generic: `NMMusicPlayer<T>`. Extend `MusicPlaylistItem` with your own fields and pass the extended type at construction:

```ts
import nmMPlayer from '@nomercy-entertainment/nomercy-music-player';
import type { MusicPlaylistItem } from '@nomercy-entertainment/nomercy-music-player';

interface TrackItem extends MusicPlaylistItem {
  catalogueId: number;
  genre: string;
  isrc: string;
}

const player = nmMPlayer<TrackItem>('main');
// player.current() returns TrackItem | undefined
// player.queue() returns ReadonlyArray<TrackItem>
```

Custom fields are accessible in all event payloads (`current`, `crossfadeStart`, `trackEndingSoon`) and inside plugins via `this.player.current()`.

---

## Full example

```ts
const BASE = 'https://raw.githubusercontent.com/NoMercy-Entertainment/nomercy-media/master/Music';
const D = `${BASE}/D/Derek%20Clegg/%5B2010%5D%20KJC`;

const playlist: MusicPlaylistItem[] = [
  {
    id: 'kjc-01',
    name: 'Thaw You Out',
    url: `${D}/01%20Thaw%20You%20Out.mp3`,
    cover: `${D}/01%20Thaw%20You%20Out.jpg`,
    lyricsUrl: `${D}/01%20Thaw%20You%20Out.lrc`,
    artist_track: [{ id: 1, name: 'Derek Clegg' }],
    album_track: [{ id: 1, name: 'KJC' }],
    duration: 204,
  },
  {
    id: 'kjc-02',
    name: 'Hope',
    url: `${D}/02%20Hope.mp3`,
    cover: `${D}/02%20Hope.jpg`,
    lyricsUrl: `${D}/02%20Hope.lrc`,
    artist_track: [{ id: 1, name: 'Derek Clegg' }],
    album_track: [{ id: 1, name: 'KJC' }],
    duration: 198,
  },
];
```

---

## See also

- [Configuration](/nomercy-music-player/configuration) — `playlist` option that accepts these items
- [Events](/nomercy-music-player/events) — `current` event payload includes the item
- [LyricsPlugin](/nomercy-music-player/plugins/lyrics) — reads `lyricsUrl` on each track change
- [MediaSessionPlugin](/nomercy-music-player/plugins/media-session) — reads `name`, `artist_track`, `album_track`, `cover`
