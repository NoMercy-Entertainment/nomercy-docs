...
The kit uses this value whenever `clockSource` is not configured.
...
This sync uses the platform `fetch` on purpose, not the kit's built-in `authFetch`, because the round-trip measurement must not be thrown off by automatic retries.
That is the exception, not the rule.
For ordinary data requests, prefer the built-in client: `authFetch` outside a plugin, or `this.fetch` inside one, so your auth and retry policy are applied for you. See [Auth and Fetch](/nomercy-player-kit/auth-fetch).
...
- [Realtime](/nomercy-player-kit/adapters/realtime), the WebSocket channel port, used by group-listening features that pair with a corrected clock
