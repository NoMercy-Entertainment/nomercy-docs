...
The token fetch uses `authFetch`, the kit's built-in client, so it carries your auth and retry policy automatically.
Inside a plugin the same client is `this.fetch`, and you should prefer it over the platform `fetch` by default. See [Auth and Fetch](/nomercy-player-kit/auth-fetch).
