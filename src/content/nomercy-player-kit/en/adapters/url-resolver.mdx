...
The player core passes a category string alongside every URL so your resolver can branch on asset type.
...
Inside a plugin the same client is `this.fetch`, and you should prefer it over the platform `fetch` by default. See [Auth and Fetch](/nomercy-player-core/auth-fetch).
...
- [Logger](/nomercy-player-core/adapters/logger), the logging port
