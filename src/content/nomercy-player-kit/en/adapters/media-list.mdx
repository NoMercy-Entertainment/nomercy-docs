...
- `index()` is `0` when items exist and `-1` when the list is empty.
...
Reach for the built-in one by default and only drop to the platform `fetch` when you have a reason to. See [Auth and Fetch](/nomercy-player-core/auth-fetch).

## See also

- [Adapters](/nomercy-player-core/adapters), the full port catalog
- [Event Bus](/nomercy-player-core/adapters/event-bus), `MediaList` extends `EventEmitter` directly
