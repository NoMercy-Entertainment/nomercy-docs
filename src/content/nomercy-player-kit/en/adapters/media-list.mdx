...
`authFetch` is the kit's built-in client, so your bearer token, retry policy, and `fetch:*` events all come along for free.
Inside a plugin you reach the exact same client through `this.fetch`.
Reach for the built-in one by default and only drop to the platform `fetch` when you have a reason to. See [Auth and Fetch](/nomercy-player-kit/auth-fetch).
