...
All registered plugin instances in registration order (both enabled and disabled).
Returns a fresh array, so mutating it does not affect the player's internal list.
...
- [Built-in Core Plugins](/nomercy-player-core/plugins-builtin): shipped plugins
- [Events Reference](/nomercy-player-core/events-reference): `plugin:installed`, `plugin:failed`, `plugin:disposed`
