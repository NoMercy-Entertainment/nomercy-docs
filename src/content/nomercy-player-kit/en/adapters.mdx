---
title: 'Adapters'
description: 'The core adapter ports. Name, purpose, default implementation, and how to replace each one.'
---
...
This is also the line between an adapter and a plugin. If you're changing how something the player already does works on the inside, that's a port swap. If you're adding something new for the player to do, like an equalizer or a skip button, that's a [plugin](/nomercy-player-core/plugins). [Plugins and adapters](/nomercy-player-core/foundations/plugins-and-adapters) covers when to reach for each.
...
Libraries built on the core add ports of their own. Those are documented with the library that owns them: see [video configuration](/nomercy-video-player/configuration) and [music configuration](/nomercy-music-player/configuration).
