...
**Returns:** `PlayStateToken` — `'playing' | 'paused' | 'stopped' | 'idle'`
...
**Returns:** `VolumeStateToken` — `'unmuted' | 'muted'`
...
**Returns:** `RepeatStateToken | void` — `'none' | 'one' | 'all'`
...
Accepts a `ShuffleStateToken` or a plain boolean (`true` → `'on'`, `false` → `'off'`).
...
`playState` and `volumeState` are read-only getters (no before-mutation guard). `repeatState` and `shuffleState` write mutations go through `_emitBeforeMutation` when `mutationGuards` is not `false`.

`currentTime`, `volume`, and `playbackRate` are classified as hot mutations and skip the guard by default — they fire too often. Pass `mutationGuards: 'all'` to `setup()` to guard them during debugging.

---

## See also

- [Volume](/nomercy-player-kit/volume) — `volume()`, `mute()`, `unmute()`
- [Transport](/nomercy-player-kit/transport) — `play()`, `pause()`, `stop()`, `next()`
- [Events Reference](/nomercy-player-kit/events-reference) — `repeat`, `shuffle`, `play`, `pause`
