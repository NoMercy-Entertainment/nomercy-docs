...
Lightweight scoped logger. The kit and every plugin share a single root instance per player.
...
Pass a custom logger via `setup({ logger })` to replace the kit's default implementation. The `ILogger` interface is the full contract.
...
Plugin authors never construct a logger directly — they receive `this.logger` from the kit, already scoped to their plugin id.
...
| `minCoreVersion` | No          | Minimum kit version (semver). Checked at registration.                                  |
...
this.on(EqualizerPlugin, 'bandChanged', (data) => {
  console.log(data.index, data.gain);
});
```
...
- [Plugin Registration](/nomercy-player-kit/plugin-registration) — `addPlugin`, `removePlugin`, `getPlugin`
- [Built-in Plugins](/nomercy-player-kit/plugins-builtin) — plugins shipped with the kit
- [Key Interfaces](/nomercy-player-kit/types/interfaces) — `IPlayer`, `BasePlayerConfig`, `BaseEventMap`
- [Storage Adapter](/nomercy-player-kit/storage) — `IStorage` and backend implementations
