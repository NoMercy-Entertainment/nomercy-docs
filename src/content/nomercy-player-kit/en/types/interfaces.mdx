...
interface IPlayer<E extends BaseEventMap = BaseEventMap> extends DispatchTarget {
  ...
  // Cue parsers
  registerICueParser(parser: ICueParser, prepend?: boolean): void;
  unregisterICueParser(id: string): void;
  resolveICueParser(url: string): ICueParser | undefined;
...
`IPlayer<E>` is intentionally item-type-agnostic; `current()` is not on it.
