...
const restore = player.experimental.override('play', async function (...args) {
  ...
  const orig = player.experimental.overrides().find((o) => o.method === 'play');
...
Snapshot of all active overrides. Each entry names the method and which layer installed it (`'consumer'` for app code, or a plugin id).
...
When a plugin that installed an override is disposed (via `removePlugin`), its overrides are restored automatically. Consumer-installed overrides (`by: 'consumer'`) are not auto-restored.

---

## See also

- [Events Reference](/nomercy-player-kit/events-reference) — `beforePlay`, `beforeSeek`, and other cancellable events (prefer these over `override`)
- [Mutation Guards](/nomercy-player-kit/mutation-guards) — `beforeMutation` advisory system
