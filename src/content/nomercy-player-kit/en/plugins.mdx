...
description: 'What plugins are, the five-layer model, how core plugins differ from library plugins and consumer plugins, and where to find each.'
---
...
The [five-layer model](/nomercy-player-core/architecture) determines where a plugin belongs:
...
See [Built-in Core Plugins](/nomercy-player-core/plugins-builtin) for the full catalogue.
...
See [Plugin Authoring](/nomercy-player-core/plugin-authoring) for how to write one.
...
For the full registration API see [Plugin Registration](/nomercy-player-core/plugin-registration).

## Plugin contract

Every plugin must satisfy the conformance checklist in the [Plugin Standard](/nomercy-player-core/plugin-standard). The standard is enforced by lint rules in the core.
...
Quick rule: adding something new the player does is a plugin; changing how it already does something is an adapter (a port swap). They pair up nicely too, like a plugin that reads from a storage port you swapped. [Plugins and adapters](/nomercy-player-core/foundations/plugins-and-adapters) walks through the boundary and when to reach for each.

## Where to go next

| Goal | Page |
| --- | --- |
| Register, look up, or remove a plugin | [Plugin Registration](/nomercy-player-core/plugin-registration) |
| Browse all core-shipped plugins | [Built-in Core Plugins](/nomercy-player-core/plugins-builtin) |
| Write your own plugin | [Plugin Authoring](/nomercy-player-core/plugin-authoring) |
| Read the locked conformance standard | [Plugin Standard](/nomercy-player-core/plugin-standard) |
| Browse adapter ports | [Adapter Ports](/nomercy-player-core/adapters) |
