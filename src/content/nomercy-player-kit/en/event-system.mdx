...
Inside a plugin body, the class-form provides full type inference via the plugin's `E` generic. This form is `protected` — it is only available inside `Plugin` subclasses, not on an external `player.on()` call:
