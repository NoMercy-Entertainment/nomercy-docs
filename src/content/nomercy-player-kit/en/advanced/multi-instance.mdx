...
players[0].on('play', () => {
  ...
  players.slice(1).forEach((p) => {
    p.currentTime(time); // sync position
    p.play();
  });
});

players[0].on('pause', () => {
  players.slice(1).forEach((p) => p.pause());
});
```
...
In practice, use the [WatchPartyLitePlugin](/nomercy-player-kit/advanced/custom-plugin#watch-party-lite) worked example as a starting point and extend it with a leader election algorithm.
...
Each player's storage is namespaced by its `id`. Two players with ids `'main'` and `'pip'` write to different storage keys:
...
- [Advanced: Custom Plugin](/nomercy-player-kit/advanced/custom-plugin) — the watch-party-lite plugin for cross-instance sync
