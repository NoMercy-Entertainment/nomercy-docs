...
Read this when you are building a production plugin — something that will ship to users, run in multiple player instances, or coordinate across components. The [Plugin Authoring](/nomercy-player-kit/plugin-authoring) page covers the basics. This page covers the hard parts.
...
describePlugin(
  ...
  { roomId: 'test-room', role: 'leader' },
  ({ player, plugin }) => {
    test('broadcasts play events', () => {
      const synced: unknown[] = [];
      player.on('plugin:watch-party-lite:synced', (e) => synced.push(e));
...
- [Plugin Authoring](/nomercy-player-kit/plugin-authoring) — the complete plugin contract reference
- [Plugin Standard](/nomercy-player-kit/plugin-standard) — the locked conformance rules
- [Advanced: Multi-Instance](/nomercy-player-kit/advanced/multi-instance) — coordinating multiple player instances at the page level
- [Kit — Testing](/nomercy-player-kit/testing) — full test harness reference
