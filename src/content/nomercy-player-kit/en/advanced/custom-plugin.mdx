...
Read this when you are building a production plugin, something that will ship to users, run in multiple player instances, or coordinate across components.
...
Advisories let the player core warn in development when a method is called at the wrong lifecycle phase:
...
In `NODE_ENV=development`, the player core emits a console warning.
In production, advisories are silent.
...
describePlugin(
  ...
  ({ player, plugin }) => {
    test('broadcasts play events', () => {
      ...
      player.on('plugin:watch-party-lite:synced', (event) => synced.push(event));
