...
Read this when the default adapter implementations don't fit your use case, for example when you need async persistence (IndexedDB Workers), translations from a CMS, or any other infrastructure replacement.

**Prerequisites:** [Core: Adapters](/nomercy-player-core/adapters) for the port catalog. [Core: Testing](/nomercy-player-core/testing) for the test harness.

## What an adapter is

An adapter is a class that implements a typed interface from the core.
You wire it through `setup()` and the player and its plugins use your implementation instead of the default.

There are 28 core-level ports plus 4 video-specific and 6 music-specific ones.
This guide walks through two of the most commonly replaced ones:
...
For async backends (IndexedDB, remote API), you need to either:

- Use a write-through cache that flushes asynchronously
- Use `IndexedDBBackend` (ships with the core) if standard IndexedDB fits
...
If your adapter can fail (network error, worker crash), do not throw.
Return `null` from `get` and silently fail on `set`.
The player is resilient to storage failures:
...
Some adapter interfaces include lifecycle callbacks.
For example, `IRealtimeChannel` has `open()` / `close()` methods called by the player's lifecycle.
Always implement them even if your adapter does not need them, the player calls them unconditionally.
...
Use `MemoryStorageBackend` as the baseline in tests, it satisfies `IStorage` without any browser APIs.
Test your custom backend in isolation:
...
- [Core: Adapters](/nomercy-player-core/adapters): full port catalog with all 28 interfaces
- [Core: Testing](/nomercy-player-core/testing): `describePlugin`, `assertNoListenerLeak`, and the full test harness
- [Advanced: Custom Plugin](/nomercy-player-core/advanced/custom-plugin): writing a plugin that uses your custom adapter
