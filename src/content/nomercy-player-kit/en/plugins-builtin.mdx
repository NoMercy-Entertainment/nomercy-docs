...
Mounts a single `<canvas>` into the player container and runs the shared RAF render loop. Visualization plugins register per-frame callbacks via `addRenderer()` rather than managing their own canvases. Without this plugin no canvas is created and no animation frames are requested — zero cost when not used.
...
| `compositeMode` | `'clear' \| 'composite'` | `'clear'` | `'clear'` — canvas is cleared at the start of each frame. `'composite'` — renderers accumulate on top of each other. |
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `chromecastAppId` | `string` | — | Chromecast app id passed to the Cast SDK. |
| `enableAirPlay` | `boolean` | — | Whether AirPlay is allowed (Safari only). |
| `customReceiverNamespace` | `string` | — | Custom receiver namespace for messages. |
...
10-band parametric EQ with pre-gain, built-in named presets, custom presets, and persistence. Inserts a `GainNode` (pre-gain) followed by 10 peaking `BiquadFilterNode`s into the audio graph as `'post'` effects. Requires `AudioGraphPlugin`.
...
| `preset` | `string` | — | Named preset to apply immediately on `use()`. |
| `presets` | `ReadonlyArray<EqPreset>` | — | Additional or replacement presets merged into the catalogue. |
| `sliderValues` | `EqSliderValues` | built-in ±12 dB ranges | Override min/max/step ranges for slider helper methods. |
| `persistKey` | `string` | — | Storage key for automatic persistence of bands, active preset, and custom presets. |
...
| `persistKey` | `string` | — | Storage key for automatic persistence of gain, pan, and mute state. |
...
| Option | Type | Default | Description |
| --- | --- | --- | --- |
| `allowedOrigins` | `string \| string[]` | — | Allowed origin(s) for inbound `nm:command` messages. `'*'` accepts all (development only). Omitting the option rejects all inbound commands. |
...
window.addEventListener('message', (e) => {
  if (e.data.type === 'nm:event') console.log(e.data.name, e.data);
});
```
...
| `bindings` | `Record<string, (player: P) => void>` | — | Extra bindings merged on top of defaults. Same-combo entries here win. |
...
| `when` | `(e: KeyboardEvent) => boolean` | — | Gate predicate. Return `false` to suppress all key handling for that event (e.g. during modals). |
...
Automatically wires on `use()`: `current` → pushes metadata; `play`/`pause`/`ended` → `playbackState`; `time`/`seek` → `setPositionState`. OS action handlers registered: `play`, `pause`, `stop`, `previoustrack`, `nexttrack`, `seekbackward`, `seekforward`, `seekto`.
...
Two modes: **transient** (`show`, `queue`) — auto-hides after a duration; **persistent** (`displayPersistent`, `removePersistent`) — stays until explicitly removed.
...
| `mountSelector` | `string` | — | CSS selector for an existing element in which to mount the toast surface. When absent, a `<div>` is created inside the player container. |
...
Browser support: Web Locks (Chrome 69+, Firefox 96+, Safari 15.4+). In environments without `navigator.locks` the plugin emits `unsupported` and becomes a no-op — playback is unaffected.
