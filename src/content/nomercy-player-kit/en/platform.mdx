...
- [setup()](/player/kit/setup) — `platform` config field, `wakeLock` policy
- [Device Detection](/player/kit/device) — `device()`, `DeviceCapabilities`
- [Events Reference](/player/kit/events-reference) — `network:online`, `network:offline`, `network:slow`, `visibility:visible`, `visibility:hidden`
