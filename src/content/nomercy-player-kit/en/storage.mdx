...
The storage adapter gives plugins a namespaced, async-compatible key-value store. Each plugin receives an auto-namespaced wrapper around the shared backend — plugin keys never collide with each other or with player-level keys.
...
Methods may return values or Promises — synchronous backends (`localStorage`) and asynchronous ones (IndexedDB, remote API) share one interface. Plugin code always uses `await`.
...
Inside a plugin, `this.storage` is the auto-namespaced wrapper — no need to prefix keys:
...
class RemoteStorage implements IStorage {
  async get(key: string) {
    const r = await fetch(`/api/prefs/${key}`, { credentials: 'include' });
    if (!r.ok) return null;
    return r.text();
  }
...
- [setup()](/nomercy-player-kit/setup) — `storage` config field
- [Plugin Registration](/nomercy-player-kit/plugin-registration) — plugin `static` contract
