...
**Prerequisites:** [Kit — Adapters](/player/kit/adapters) covers the three built-in storage backends.
...
| SSR (Next.js, Nuxt)     | `MemoryStorageBackend` on server, `LocalStorageBackend` on client — see [Advanced: SSR](/player/advanced/server-side-rendering) |
...
`IndexedDBBackend` is async internally but the `IStorage` interface is synchronous. Writes are fire-and-forget; reads return the last committed value. For a fully async custom backend, see [Advanced: Custom Adapter](/player/advanced/custom-adapter).

---

## What to read next

- [Kit — Adapters](/player/kit/adapters) — full `IStorage` interface and the backend catalog
- [Advanced: Custom Adapter](/player/advanced/custom-adapter) — writing an async IndexedDB-Worker backend
- [Recipes: Lyrics and Equalizer](/player/recipes/lyrics-and-equalizer) — EQ state is persisted through this same storage system
