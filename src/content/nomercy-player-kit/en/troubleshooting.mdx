...
**Cause:** The kit doesn't auto-persist volume — that's a plugin concern.

**Fix:** Persist it in a small plugin via the namespaced [storage adapter](/nomercy-player-kit/storage) (`this.storage`), not raw `localStorage`. Volume is on the **0–100** scale.
