...
description: 'Common setup and runtime problems with the NoMercy Player SDK. Symptoms, causes, and fixes, verified against core source.'
---

# Troubleshooting

Every entry below is verified against the core source.
If the symptom you're hitting isn't here, file an issue with a minimal reproduction.

## Octopus (ASS/SSA) subtitles do not render

**Symptom:** ASS/SSA subtitles are enabled and selected, but nothing appears on screen.
...
For a single-player app, picking a static id is fine.
For multi-player UIs (PiP, watch parties, tile grids), generate a stable id per slot.
...
**Fix:** Persist it in a small plugin via the namespaced [storage adapter](/nomercy-player-core/storage) (`this.storage`), not raw `localStorage`.
...
**Fix:** Don't register them yet.
Track progress in [Versioning](/nomercy-player-core/versioning).

## HLS auth: segments return 401 after a delay

**Symptom:** HLS manifest loads, playback starts, then segment requests start returning 401 after some time.
Or every segment 401s from the first request.
...
**Cause:** `static onError` is a declarative map: `{ 'error-code': { action: 'disable' | 'retry' | 'fallback' } }`.
...
See [Plugin Standard §1.8](/nomercy-player-core/plugin-standard) for the full error escalation contract.
