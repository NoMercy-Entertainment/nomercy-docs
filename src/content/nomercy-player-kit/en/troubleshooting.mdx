...
**Fix:** `@nomercy-entertainment/nomercy-subtitle-octopus` is a regular dependency of `@nomercy-entertainment/nomercy-video-player` — it should already be present after `npm install`. If your bundler tree-shakes it out or you are using a selective install, add it explicitly:
...
**Fix:** Don't register them yet. Track progress in [Versioning](/nomercy-player-kit/versioning).
...
See [Plugin Standard §1.8](/nomercy-player-kit/plugin-standard) for the full error escalation contract.
