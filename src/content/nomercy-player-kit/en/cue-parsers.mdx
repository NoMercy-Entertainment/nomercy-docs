...
| ----------- | -------- | ------- | ----------------------------------------- |
| `trackerId` | `string` | none    | Stable identifier (for debugging)         |
...
player.registerCueParser({
  ...
  async parse(url) {
    const response = await this.fetch(url, { responseType: 'text' });
    return parseTTML(response);
  },
});
```
...
- [Media Tracks](/nomercy-player-core/media-tracks): `currentSubtitle`, sidecar VTT handling
