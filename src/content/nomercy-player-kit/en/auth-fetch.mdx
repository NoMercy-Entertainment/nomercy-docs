...
Must return a new valid token.
The failed request is retried exactly once with the new token.
...
The default retry policy retries failed network requests with exponential backoff.
Configure it by passing a partial `IRetryPolicy` map to `setup()`, it is merged over `DEFAULT_RETRY_POLICY`, so only the codes you want to override need to appear:
