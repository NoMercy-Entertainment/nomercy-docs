...
**Writing** installs a custom estimator.
The active stream source calls the function on every ABR level decision.
Install your own estimator when you have out-of-band signal (SignalR quality report, server-sent bitrate hint) that is more accurate than the default download-speed measurement.
...
- [Media Tracks](/nomercy-player-core/media-tracks): `qualityLevels()` and `currentQuality()`
- [Player State](/nomercy-player-core/player-state): `qualityState()` for the ABR/MANUAL mode flag
- [Metrics](/nomercy-player-core/metrics): `avgBitrate` in `PlaybackMetrics`
- [Platform](/nomercy-player-core/platform): the `capabilities.canDecode` platform port
