...
The full `BasePlayerConfig` is documented in [Key Interfaces — BasePlayerConfig](/player/kit/types/interfaces). Most common fields:
...
const player = nmplayer('main').setup({
  baseUrl: 'https://media.nomercy.tv',
  auth: { bearerToken: () => myAuth.getToken() },
  playlist: [
    { id: '1', title: 'Sintel', url: '/sintel/master.m3u8' },
  ],
});
...
- [Phase API](/player/kit/phase) — `phase()`, `dispatching()`
- [dispose()](/player/kit/dispose) — teardown
- [Key Interfaces — BasePlayerConfig](/player/kit/types/interfaces)
- [Events Reference](/player/kit/events-reference) — setup lifecycle events
