...
| `setupStart`         | `setupStartError`         | Phase → `setup`, container element resolved                          |
...
The full `BasePlayerConfig` is documented in [Key Interfaces — BasePlayerConfig](/nomercy-player-kit/types/interfaces). Most common fields:

| Field                | Type                                         | Default               | Description                                        |
| -------------------- | -------------------------------------------- | --------------------- | -------------------------------------------------- |
| `baseUrl`            | `string`                                     | —                     | Prepended to relative media URLs in playlist items |
| `logLevel`           | `'debug' \| 'info' \| 'warn' \| 'error'`     | `'warn'`              | Log verbosity                                      |
| `auth`               | `AuthConfig`                                 | —                     | Bearer token, headers, refresh function            |
| `playlist`           | `BasePlaylistItem[] \| string`               | —                     | Initial playlist (items or URL to fetch)           |
...
| `wakeLock`           | `'auto' \| 'always' \| 'never'`              | `'never'`             | Screen wake-lock policy                            |
...
| `cast`               | `CastConfig`                                 | —                     | Chromecast configuration                           |

---

## `ready()`

Returns a promise that resolves when the setup pipeline reaches `ready`, or rejects if any stage fails or `dispose()` is called first. Memoised — repeated calls return the same promise.

```ts
const player = nmplayer('main').setup({
  baseUrl: 'https://raw.githubusercontent.com/NoMercy-Entertainment/nomercy-media/master/Films',
  auth: { bearerToken: () => myAuth.getToken() },
  playlist: [{ id: '1', title: 'Sintel', url: '/sintel/master.m3u8' }],
});
...
- [Phase API](/nomercy-player-kit/phase) — `phase()`, `dispatching()`
- [dispose()](/nomercy-player-kit/dispose) — teardown
- [Key Interfaces — BasePlayerConfig](/nomercy-player-kit/types/interfaces)
- [Events Reference](/nomercy-player-kit/events-reference) — setup lifecycle events
