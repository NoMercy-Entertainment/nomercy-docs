...
The player core ships a Vite plugin that rewrites `translationsFromGlob('./i18n/*.ts')`
call sites into the `import.meta.glob` form Vite's static analyser requires.
Without it, any code that calls the helper with a literal string (the built-in
UI plugins do) throws a `StateError` at runtime.

Two pieces are needed in your `vite.config.ts`. Register the plugin, and keep
the player packages out of `optimizeDeps` (esbuild pre-bundling bypasses Vite
transforms, so a pre-bundled player never gets rewritten):
...
You only need this when code calls `translationsFromGlob` with a string pattern.
