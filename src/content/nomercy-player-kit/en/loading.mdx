...
| --------------- | --------------------- | ----------- | ----------------------------------------------- |
| `item.url`      | `string`              | —           | Required. The playable media URL                |
| `opts.slot`     | `'current' \| 'next'` | `'current'` | `'next'` preloads without interrupting playback |
| `opts.startAt`  | `number`              | —           | Start position in seconds                       |
| `opts.fadeIn`   | `number`              | —           | Volume fade-in duration in seconds              |
...
- [Media Tracks](/nomercy-player-kit/media-tracks) — `subtitles()`, `chapters()`, `currentSubtitle()`
- [Transport](/nomercy-player-kit/transport) — `play()`, `stop()`
- [Events Reference](/nomercy-player-kit/events-reference) — `beforeLoad`, `mediaReady`, `playlistReady`
