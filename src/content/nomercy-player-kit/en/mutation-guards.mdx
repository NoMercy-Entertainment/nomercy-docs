...
4. If any `beforeMutation` listener called `event.preventDefault()`, emits `mutationPrevented` and returns `false` — the mutation is skipped.
...
- [Plugin Registration](/nomercy-player-kit/plugin-registration) — `addPlugin`, `static advisories`
- [Events Reference](/nomercy-player-kit/events-reference) — `beforeMutation`, `mutationPrevented`
