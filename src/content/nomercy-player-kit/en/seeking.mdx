...
**Write:** `currentTime(t, opts?)` dispatches `beforeSeek`; if not prevented, runs a `seeking` phase round-trip, updates position, emits `seek` then `seeked`, and forwards to the backend.
...
Delegates to `currentTime()` so `beforeSeek` fires and the full seek cycle applies.
