...
The event bridge is wired lazily on the first read or write — it is idempotent to call queue methods before `setup()`.
...
player.queue([
  { id: 'sintel', title: 'Sintel', image: 'https://cdn.nomercy.tv/assets/sintel.jpg' },
  { id: 'bbb', title: 'Big Buck Bunny', image: 'https://cdn.nomercy.tv/assets/bbb.jpg' },
]);
```
...
**Events emitted:** `queue:append` with `{ items, from }` — `from` is the index of the first appended item
...
| Parameter | Type               | Description                                |
| --------- | ------------------ | ------------------------------------------ |
| `compare` | `(a, b) => number` | Comparator — negative means `a` before `b` |

**Events emitted:** `queue:sort`

```ts
// alphabetical by title
player.queueSort((a, b) => (a.title ?? '').localeCompare(b.title ?? ''));
```
...
- [Queue Navigation](/nomercy-player-kit/queue-navigation) — `current`, `next`, `previous`, `seekToIndex`
- [Backlog](/nomercy-player-kit/backlog) — `backlog`, `backlogAppend`
- [BasePlaylistItem](/nomercy-player-kit/playlist-item) — item shape
- [Events Reference](/nomercy-player-kit/events-reference) — `queue`, `queue:append`, `queue:exhausted`
