...
The preload and transition system controls what happens at the boundary between two queue items.
The preload strategy decides when and what to prefetch.
The transition strategy decides how the handoff from outgoing to incoming item is handled.
...
player.setup({
  preloadStrategy: {
    shouldPreload({ time, duration }) {
      return duration > 0 && duration - time < 20;
    },
...
interface TransitionStrategy {
  ...
  execute(context: TransitionContext, backend: TransitionBackend): Promise<void>;
  cancel(): void;
}
```
...
The dual-playback surface required by the transition engine.
Audio backends that support crossfade implement this:
...
Hard-cut, no audio overlap.
Default for the video player.
Items are still preloaded; there is no audio crossfade.
...
- [setup()](/nomercy-player-core/setup): `preloadLeadSeconds`, `crossfadeEnabled`
- [Events Reference](/nomercy-player-core/events-reference): preload and transition events
