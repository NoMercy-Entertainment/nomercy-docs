...
`this.fetch(...)` (auth-aware HTTP) and `this.resolveUrl(...)` (auth-aware URL resolution) are covered under [Auth and Fetch](/nomercy-player-core/auth-fetch); `this.t(...)` under [Internationalisation](/nomercy-player-core/i18n).
