...
On the host page, listen for `message` events with `event.data.type === 'nm:event'` and post commands shaped like `EmbedCommand` (`{ type: 'play' }`, `{ type: 'seek', time: 30 }`) to the iframe's `contentWindow`.
