...
- Safari specifically requires the `autoplay` attribute on the host `<iframe>` (when embedded) for MediaSession to activate at all, see the note on the [Embed](/nomercy-player-core/plugins-adapters/embed) plugin.
