...
`RealtimeFactoryOptions`: `protocols` (WebSocket sub-protocol strings), `reconnect` / `baseDelayMs` / `maxDelayMs` (reconnection knobs forwarded to the factory; only a factory that implements reconnection reads them, and the built-in native adapter does not), and `factory` (a per-call override, one connection using a different transport without changing the global default).
