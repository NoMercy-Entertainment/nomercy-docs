...
{/* Source: packages/player-web/nomercy-player-core/src/types/config.ts */}

# Reference: BasePlayerConfig

The object passed to `setup(config)`. Everything below is shared by both `nomercy-video-player` and `nomercy-music-player`, and each library extends it with its own domain fields:

- `nomercy-video-player` adds `muted`, `autoPlay`, `stretching`, `playbackRates`, `preload`, `disableMediaControls`, `disableControls`, `backendFactory`, `defaultSubtitleLanguage`, `defaultAudioLanguage`, `defaultQuality`, `theaterDefault`, and `autoAdvance`.
- `nomercy-music-player` adds `backend`, `backendFactory`, and `crossfadeDefaults`.
