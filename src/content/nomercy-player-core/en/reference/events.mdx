...
| `time` | `{ time: number }` — unthrottled, per-frame. |
| `dispose` | `void` |
...
Subtitle-specific rows (`subtitleCue`, `subtitleStyle`, `subtitle`, `beforeSubtitle` / `subtitlePrevented`) live in `BaseEventMap` for shared-implementation reasons, but subtitles are a video-only concern, see [Reference: Events](/nomercy-video-player/reference/events) on the video player for that table.
