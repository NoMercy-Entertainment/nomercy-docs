...
## Cues, subtitles, audio track, chapters, cast

| Event | Payload |
|---|---|
| `cue:enter` / `cue:exit` | `CueEventPayload` |
| `subtitleCue` | `SubtitleCueChange` — unified across sidecar VTT and native text tracks. |
| `subtitleStyle` | `SubtitleStyle` |
| `subtitle` | `{ track: number \| null }` |
| `beforeSubtitle` / `subtitlePrevented` | `BeforeEvent<{ track }>` / `{ reason, cause? }` |
