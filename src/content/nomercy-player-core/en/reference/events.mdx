...
{/* Source: packages/player-web/nomercy-player-core/src/types/events.ts */}

# Reference: Events

`BaseEventMap` is the complete event map every player built on the kit emits. Subscribe with `player.on(name, handler)`. The payload type is inferred from the name.

Library-specific maps (`VideoEventMap`, `MusicEventMap`) extend this with domain-only events on top.
...
Subtitle-specific rows (`subtitleCue`, `subtitleStyle`, `subtitle`, `beforeSubtitle` / `subtitlePrevented`, `subtitles`) are typed in `BaseEventMap` at the core level, for shared-implementation reasons. In practice, subtitles are a video-only concern.

See [Reference: Events](/nomercy-video-player/reference/events) on the video player for the full walkthrough.
