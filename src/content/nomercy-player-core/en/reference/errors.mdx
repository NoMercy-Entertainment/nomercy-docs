...
{/* Source: packages/player-web/nomercy-player-core/src/errors/*.ts, src/adapters/retry-policy/default.ts */}
...
An 8-digit numeric form exists alongside them for compact log storage, shaped `VVV S CC EE`. Each segment has its own range:
...
Three functions work with the numeric form:

| Function | What it does |
|---|---|
| `makeCode(fields)` | Builds one. |
| `parseCode(code)` | Decomposes it back into fields. |
| `formatCode(code)` | Zero-pads it to 8 characters. |

`VENDOR` reserves three values: `KIT: 1`, `MUSIC: 2`, and `VIDEO: 3`. A third-party plugin author registers a vendor block by PR to the kit's registry, or skips registration and lives on string codes.

## Default retry policy

`DEFAULT_RETRY_POLICY: IRetryPolicy` is a `Record<string, RetryConfig>`, an exported preset table keyed by precedence from most to least specific: exact code, then category prefix, then HTTP century (`'4xx'`/`'5xx'`), then the `'*'` wildcard.

Neither `authFetch` nor `Plugin.fetch` consults it automatically. `Plugin.fetch()` defaults to `{ attempts: 0 }` (no retry) unless you pass a `RetryConfig` explicitly.
