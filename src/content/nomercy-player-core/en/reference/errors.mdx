...
| `ResourceError` | `PlayerError` | `resourceError(...)` | A sidecar (subtitle, sprite, chapter file) failed to load. |
| `PluginError` | `PlayerError` | `pluginError(...)` | Missing dependency, version mismatch, `use()` rejection. |
| `BrowserPolicyError` | `PlayerError` | `browserPolicyError(...)` | An unsupported browser API (wake lock, fullscreen, PiP, canvas 2D, IndexedDB, Web Locks, Cast). |
...
`DEFAULT_RETRY_POLICY: IRetryPolicy` is a `Record<string, RetryConfig>`, resolved most-specific-first (exact code → category prefix → HTTP century `'4xx'`/`'5xx'` → `'*'` wildcard). This is the table `authFetch` (and `Plugin.fetch`) consult automatically.
