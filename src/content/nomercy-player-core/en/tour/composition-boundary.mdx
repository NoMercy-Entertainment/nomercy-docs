...
`nomercy-video-player` and `nomercy-music-player` do not share code by importing each other. They share it because both *are* `nomercy-player-core` composed onto their own prototype. The core mixins are `this` for both players at once, which is exactly why a fix there reaches both in one release, and exactly why the [core rules](/nomercy-player-core) say a method that would branch on "is this music or video" does not belong in core: a branch means the behaviour is not really shared, so it is not really part of the thing both players are.
