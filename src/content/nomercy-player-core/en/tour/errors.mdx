...
`DEFAULT_RETRY_POLICY` maps error codes to a `RetryConfig` (`attempts`, `backoff`, `baseMs`, `maxMs`, `refreshFirst`), resolved most-specific-first: exact code, then category prefix, then an HTTP range (`'4xx'` / `'5xx'`), then a `'*'` wildcard. It's the table the kit's own `authFetch` pipeline (and `Plugin.fetch`) consults automatically, a `core:auth/forbidden` never retries, a `core:network/timeout` retries five times with exponential backoff. Pass an entry from this map, or your own `RetryConfig`, as the per-call `retry` option on `Plugin.fetch()` to override the default for one call site.
...
- [What is nomercy-player-core](/nomercy-player-core/introduction): back to the start, the full "what you get" list this tour walked through one concept at a time.
