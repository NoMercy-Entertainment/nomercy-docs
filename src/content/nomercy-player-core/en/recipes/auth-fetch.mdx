...
A 401 triggers `auth.refreshOnUnauthenticated()` exactly once, then retries the original request with freshly resolved credentials.

A 403 propagates immediately. It is never retried and never refreshed.

5xx and network failures retry per `RetryConfig`. [Typed Errors](/nomercy-player-core/tour/errors) has the default policy table.
...
In the `'text'` branch, `T` is inferred from the `parser`'s return type, so the explicit `<string[]>` above is optional. Write `{ parser: (raw): string[] => ... }` and the fetch resolves to `Promise<string[]>` on its own.

That works because the parser is the one place your code runs to produce the value, so the compiler can hold you to its return type.

The `'json'` and `'arrayBuffer'` branches have no callback to infer from. You name `T` yourself, as `fetch<QueueSnapshot>`, and the kit casts the decoded body to it unchecked. It is the same `response.json() as T` you would write by hand, with nothing validating the shape at runtime.

So keep the explicit type parameter for bodies you trust, and reach for a `parser` when you want the transform itself type-checked.
