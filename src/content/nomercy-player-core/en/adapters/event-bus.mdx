...
- `on()` (and `off()`) fire a microtask-delayed `listeners-changed` event when an event gains its first listener or loses its last, useful for lazy-load logic.
...
- [Lifecycle Registry](/nomercy-player-core/adapters/lifecycle-registry), the other internal resource plugins depend on
