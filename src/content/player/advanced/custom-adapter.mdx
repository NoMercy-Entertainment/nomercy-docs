...
An adapter is a class that implements a typed interface from the kit. You inject it at `setup()` time and the player and its plugins use your implementation instead of the default.
...
For testing a plugin that uses your adapter, inject `MemoryStorageBackend` into the kit test harness:
