...
Read this when you are building a production plugin — something that will ship to users, run in multiple player instances, or coordinate across components. The [Plugin Authoring](/player/plugin-authoring) page covers the basics. This page covers the hard parts.

---

## The dispose contract

Every resource you acquire in `use()` must be released in `dispose()` — or through a managed helper. The kit provides helpers that auto-dispose so you don't have to track them manually:

| Resource | Managed helper | Manual alternative |
|---------|---------------|-------------------|
| Event listeners | `this.on(...)` | `this.off(...)` in `dispose()` |
| Timers | `this.timeout(ms, fn)` | `clearTimeout(id)` in `dispose()` |
| Intervals | `this.interval(ms, fn)` | `clearInterval(id)` in `dispose()` |
...
export class MyPlugin extends Plugin<NMVideoPlayer, MyOptions, {}> {
  ...
  dispose(): void {
    // Close the WebSocket manually — not tracked by the kit:
    this.ws?.close();
    this.ws = null;
...
```ts
export class PlaybackRatePlugin extends Plugin<NMVideoPlayer, PlaybackRateOptions, PlaybackRateEvents> {
  ...
  use(): void {
    ...
    const saved = this.storage.get('playback-rate');
    if (saved) {
  ...
  // Public API — getter/setter pattern mirrors the kit:
  rate(): number;
  ...
  private setRate(rate: number): void {
    this.currentRate = Math.max(0.25, Math.min(4.0, rate)); // clamp to safe range
    this.player.container.querySelector('video')?.setAttribute('playbackRate', String(this.currentRate));
...
If you need cross-instance coordination (e.g. pause all others when one starts), use the realtime channel adapter or a shared event bus:
...
Advisories let the kit warn in development when a method is called at the wrong lifecycle phase:
...
In `NODE_ENV=development`, the kit emits a console warning. In production, advisories are silent.
...
static readonly onError: Record<string, PluginRecoveryAction> = {
  'core:auth/unauthenticated': 'disable',     // disable plugin if auth fails
  'core:stream/segment-load-error': 'ignore', // don't propagate segment errors
};
```
...
export class WatchPartyLitePlugin extends Plugin<NMVideoPlayer, WatchPartyOptions, WatchPartyEvents> {
  static readonly id = 'myapp:watch-party-lite';
  static readonly version = '1.0.0';
  static readonly description = 'Sync play/pause/seek across browser tabs';

  private channel: BroadcastChannel | null = null;
  private ignoreNextEvent = false; // prevents echo loops
  ...
  private applyRemoteEvent(data: { action: 'play' | 'pause' | 'seek'; time: number }): void {
    // Set flag to prevent echo: applying a remote event will fire local events
    this.ignoreNextEvent = true;

    if (data.action === 'play') {
      this.player.currentTime(data.time); // sync position first
      this.player.play();
...
describePlugin(WatchPartyLitePlugin, { roomId: 'test-room', role: 'leader' }, ({ player, plugin }) => {
  test('broadcasts play events', () => {
    const synced: unknown[] = [];
    player.on(WatchPartyLitePlugin, 'synced', (e) => synced.push(e));
  ...
  test('no listener leak', () => {
    assertNoListenerLeak(player, WatchPartyLitePlugin);
  });
});
```
...
- [Plugin Authoring](/player/plugin-authoring) — the complete plugin contract reference
- [Plugin Standard](/player/plugin-standard) — the locked conformance rules
- [Advanced: Multi-Instance](/player/advanced/multi-instance) — coordinating multiple player instances at the page level
- [Kit — Testing](/player/kit/testing) — full test harness reference
