...
const videoPlayer = nmplayer('video').setup({
  ...
  queue: [{ id: '1', url: 'https://cdn.example.com/movie.m3u8', title: 'Movie' }],
});
videoPlayer.addPlugin(DesktopUiPlugin);

// Background music player (no UI plugin needed):
const musicPlayer = nmMPlayer('music').setup({
  queue: [{ id: '1', url: 'https://cdn.example.com/ambient.m3u8', name: 'Ambient Track' }],
});
...
In practice, use the [WatchPartyLitePlugin](/player/advanced/custom-plugin#watch-party-lite) worked example as a starting point and extend it with a leader election algorithm.
...
If you want two players to share a storage state (e.g. the same volume preference), inject the same `IStorage` backend instance and use a shared key namespace:
...
- [Advanced: Custom Plugin](/player/advanced/custom-plugin) — the watch-party-lite plugin for cross-instance sync
- [Advanced: Embedding](/player/advanced/embedding) — cross-frame (iframe) player control
- [Advanced: Performance](/player/advanced/performance) — memory budget when running multiple instances
