...
For the locked conformance document that lint rules enforce, see [Plugin Standard](/player/plugin-standard).
...
import type { NMVideoPlayer } from '@nomercy-entertainment/nomercy-video-player@beta';
...
```ts
player
  .addPlugin(ScrobblePlugin, { endpoint: '/api/v1/progress' });
