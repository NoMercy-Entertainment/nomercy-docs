...
**Symptom:** `Cannot find module 'hls.js'` at runtime, or HLS sources fail to load with no obvious error.

**Cause:** `nomercy-video-player` declares `hls.js` as an optional peer dependency. It isn't bundled. The kit only requires it when you actually load an HLS source — plain MP4 sources work without it.

**Fix:** Install hls.js alongside the player:

```
npm install hls.js
```

The kit's `peerDependencies` pins `>=1.6.0`. Any version in that range will work.
