...
hls.js is a peer dependency — install it alongside the player:
...
The player uses `hls.js`'s dynamic import internally. If `hls.js` is absent and the browser does not natively support HLS (Safari), the player falls back to a native `<video>` src load.
...
```ts
import type { IVideoBackend } from '@nomercy-entertainment/nomercy-video-player@beta';
