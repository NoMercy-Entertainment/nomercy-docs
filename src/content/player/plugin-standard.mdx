...
```ts
initialize(player, opts, lifecycle): void  // base class; rarely override
use(): void | Promise<void>                // wire listeners, mount DOM, fetch resources
dispose(): void                            // explicit teardown
```

`player.setup()` awaits all plugins' `use()` promises before emitting `ready`. A plugin whose `use()` rejects is marked failed — it does not bring down the player. Other plugins continue.
