...
setOptions(partial): void    // runtime options update
getOptions(): Readonly<O>    // current options
```
...
Options arrive via `addPlugin(Class, opts)`. Default `setOptions` shallow-merges and emits `opts:changed`. Override when you need to react to specific changes:

```ts
setOptions(patch: Partial<O>) {
...
| `plugin:<id>:opts:changed` | `{ id, opts }` | After `setOptions` |
...
this.on(EqualizerPlugin, 'bandChanged', (event) => {
  // event.data is typed from EqualizerPlugin's E generic
});
```
...
this.throw({ severity: 'fatal', code: 'my-plugin/connection-lost', message: '...' });

// Non-fatal — operation fails, plugin keeps running.
this.throw({ severity: 'error', code: 'my-plugin/fetch-failed', message: '...' });

// Degradation — plugin survives, caller continues.
this.report({ severity: 'warning', code: 'my-plugin/rate-limited', message: '...' });

// Observability.
this.report({ severity: 'info', code: 'my-plugin/cache-hit', message: '...' });
```
...
```ts
[] // no event in flight
['beforePlay'] // inside a beforePlay listener
['beforePlay', 'beforeMutation'] // nested
```
