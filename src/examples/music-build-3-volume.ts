...
/**
 * Build a Player, step 3 of 5: volume, a slider and a mute button, next to
 * Step 2's scrubber.
 *
 * Volume applies live on every `input` event (no cost to spamming it, unlike
 * seeking). The mute button reads `volumeState()` and resyncs on the
 * `'mute'` event instead of tracking its own boolean, so it can't drift when
 * something else changes mute state.
 */
