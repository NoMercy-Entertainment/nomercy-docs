...
/**
 * Build a Player, step 4 of 5: a subtitle menu next to Step 3's volume
 * control.
 *
 * `subtitle(idx)` only changes the kit's *selection state* — painting the
 * cue text on screen is a separate concern, handled by `SubtitleOverlayPlugin`
 * (`@nomercy-entertainment/nomercy-video-player/plugins/subtitle-overlay`).
 * Plugins register before `setup()` runs, so this step is the first to use
 * `configure()`, called on the freshly-constructed instance before `onReady()`.
 * The menu itself is built lazily from `subtitles()` on every open, so it
 * never needs to guess when the track list becomes available.
 */
