...
/**
 * Build a Player, step 10 of 10: Full Plugin.
 *
 * Adds to step 9: the touch layer. A 3x6 grid under the controls with
 * double-tap seek zones (with seek ripples), a center play/fullscreen zone
 * replacing the step-2 center button, and volume zones on mobile, detected
 * with `isMobile()`. Single taps dismiss the controls through the same
 * `activity` event the player's own tracker emits.
 */
...
const BIG_PLAY_ICON = '<svg viewBox="0 0 24 24" fill="currentColor" class="w-12 h-12"><path d="M5 5.27466C5 3.5678 6.82609 2.48249 8.32538 3.29828L20.687 10.0244C22.2531 10.8766 22.2531 13.125 20.687 13.9772L8.32538 20.7033C6.82609 21.5191 5 20.4338 5 18.727V5.27466Z"/></svg>';
...
class StepPlugin extends Plugin<NMVideoPlayer> {
	...
	private createTouchZones(): void {
		...
		// First child of the overlay so every control renders on top of it.
	...
	/** Wrap a double-tap handler and an optional single-tap handler, v1's exact delay logic. */
	...
	private createTouchSeekForward(pos: TouchZonePosition): void {
		const box = this.createTouchBox('seek-forward', pos);
		const ripple = this.createSeekRipple(box, 'right');
	...
	private createTouchPlayback(pos: TouchZonePosition): void {
		...
		const playButton = this.player
			...
			.appendTo(box)
			.get();
		playButton.innerHTML = BIG_PLAY_ICON;
